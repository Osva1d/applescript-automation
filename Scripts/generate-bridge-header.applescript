-- Kompletní skript: Safari ? Bridge hlavi?ka
-- Funkce pro o?i?t?ní textu
...
-- Funkce pro vytvo?ení centrované hlavi?ky Bridge
on createBridgeHeader(clientName, technology, projectNumber, lastTwoDigits)
    set totalWidth to 79  -- Courier font ?í?ka
    ...
    -- Zajistíme, ?e po?et mezer není záporn?
...
tell application "Safari"
    ...
    try
        -- Extrakce dat pomocí JavaScript a CSS selektor?
        set extractedData to do JavaScript "
            var result = {};
            
            // Extrakce ?ísla zakázky
        ...
        -- Zobrazení extrahovan?ch dat
        if projectNumber is not "" and clientName is not "" and technology is not "" then
            display dialog "Extrahovaná data ze Safari:" & return & return & "?íslo: " & projectNumber & return & "Klient: " & clientName & return & "Projekt: " & projectName & return & "Technologie: " & technology & return & return & "Vytvo?it Bridge hlavi?ku?" buttons {"Zru?it", "Ano"} default button "Ano"
            
            if button returned of result is "Ano" then
                -- Vytvo?ení Bridge hlavi?ky
                set bridgeHeader to my createBridgeHeader(clientName, technology, projectNumber, lastTwoDigits)
                
                -- Zobrazení v?sledku
                display dialog "Bridge hlavi?ka:" & return & return & bridgeHeader & return & return & "Délka: " & (length of bridgeHeader) & " znak? (cíl: 79)" & return & return & "Zkopírovat do schránky?" buttons {"Ne", "Ano"} default button "Ano"
                
                if button returned of result is "Ano" then
                    set the clipboard to bridgeHeader
                    display dialog "Hlavi?ka byla zkopírována do schránky!" & return & return & "Vlo?te ji do Adobe Bridge pomocí Cmd+V" & return & return & "D?LE?ITÉ: Ujist?te se, ?e máte nastaven? font Courier!" buttons {"OK"} default button "OK"
                end if
            end if
        else
            display dialog "Nepoda?ilo se extrahovat v?echna data:" & return & return & "?íslo: '" & projectNumber & "'" & return & "Klient: '" & clientName & "'" & return & "Technologie: '" & technology & "'" buttons {"OK"} default button "OK"
        end if
        
    on error errorMessage
        display dialog "Chyba p?i ?tení dat ze Safari:" & return & errorMessage buttons {"OK"} default button "OK"
    end try
end tell