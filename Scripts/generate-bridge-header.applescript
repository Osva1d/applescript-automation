 ...
 * 
 * Description:
 *   Bridge header generator for print production with Safari extraction.
 * ===========================================================================
 *)
...
property MIN_SPACING : 2          -- Minimum spaces between elements

-- UI Strings
property UI_BTN_OK : "OK"
property UI_BTN_CANCEL : "Zrušit"

-- Get list of legal forms to remove
on getLegalForms()
...
on processClientName(clientName, maxLength)
	...
	repeat with legalForm in legalForms
		if cleanedName ends with legalForm then
			set cleanedName to text 1 thru -(1 + (length of legalForm)) of cleanedName
		end if
	end repeat
	
	-- Clean again after legal form removal
	...
	if length of cleanedName > maxLength then
		...
		repeat with i from maxLength to 1 by -1
			if character i of cleanedName is " " then
				set lastSpacePos to i
				exit repeat
			end if
		end repeat
		
		if lastSpacePos > 0 then
			set cleanedName to text 1 thru (lastSpacePos - 1) of cleanedName
		end if
	end if
...
-- Create centered Bridge header with validation
on createBridgeHeader(clientName, technology, orderNumber, yearSuffix)
	...
	if totalContentLength > TOTAL_HEADER_WIDTH then
		-- Header too long, try to shorten client name
	...
	tell application "Safari"
		...
		try
			set extractedData to do JavaScript "
				...
				// Extract technology
				var techCell = document.querySelector('td.Task1[width=\"420\"]');
				if (techCell) {
					result.technology = techCell.textContent.replace('Technologie:', '').trim();
				}
		...
		on error errorMessage
			activate
			display dialog "Chyba při čtení dat ze Safari:" & return & errorMessage buttons {UI_BTN_OK} default button UI_BTN_OK
			return missing value
		end try
	end tell
...
on main()
	...
	if orderData is not missing value then
		...
		if orderNumber is not "" and clientName is not "" and technology is not "" then
			...
			-- Show success dialog
			activate
			display dialog "Extrahovaná data ze Safari:" & return & return & "Číslo: " & orderNumber & return & "Klient: " & clientName & return & "Technologie: " & technology & return & return & "Hlavička zkopírována do schránky!" buttons {UI_BTN_OK} default button UI_BTN_OK
		else
			activate
			display dialog "Nepodařilo se extrahovat všechna data:" & return & return & "Číslo: '" & orderNumber & "'" & return & "Klient: '" & clientName & "'" & return & "Technologie: '" & technology & "'" buttons {UI_BTN_OK} default button UI_BTN_OK
		end if
	else
		-- Warning/error already shown in extractOrderData
	end if
end main
